...
|---|---|---|
| `vti-slide-creator` | **4.9.0** | 4.8.0 (**wire the art-director engine into the entry point** — creator.py now imports + re-exports the previously-unreachable layer: `realize_archetype`/`list_archetypes` (archetypes), `make_brief`/`ration_archetypes` (art_direction), `richness_floor`/`auto_repair`/`critique`/`render_review` (visual_critic); new durable bridges `realize_layout_plan` (merges full chrome meta), `suggest_archetype` (intent→archetype, variety-aware), `richness_gate`; `CHROME_TOP_PX` synced 70→86 to match page-builder v4.5 content-area) |
| `vti-slide-creator` (prev) | 4.8.0 | 4.7.0 (**M2** `build_deck_html` injects the ECharts runtime once per deck when a chart is present; **M3** Phase 4 → DESIGN art-director — 6-step scaffold + `realize_archetype` selection/fill, Phase 5 → realize+validate, new `art_direction.py` Art Direction Brief + `ration_archetypes`; old scorer kept as image-dominant fallback) |
...
| `vti-slide-page-builder` | **4.5.0** | 4.4.0 (**proportional-fit engine** — composer tags each cell `data-fit=sparse\|normal\|dense` from content-vs-space ratio; sparse cells size-to-content + centre + scale type via `--fit-scale`, killing the hollow-stretch / spread-thin / tiny-text failure modes universally; editorial vocabulary: `--vti-fs-mega`, dark `stage`/`stage-grad` surfaces + whole-slide `slide_meta.stage`, full-bleed `slide_meta.bg_image`+scrim, 8 editorial archetypes, `stat-hero scale:mega`; stacked breadcrumb header + content-area top 70→**86px**; **removed `richness_floor` R4** (rewarded stretch) + retired char-based fill warnings → pixel ink-ratio is the density guard. **Contract:** `CHROME_TOP_PX` synced to 86 in creator `layout_designer`) |
| `vti-slide-page-builder` (prev) | 4.4.0 | 4.3.0 (finish ECharts: gauge-dial / progress-bar / funnel → ECharts gauge / track-bars / funnel; new cell `valign` option (center\|bottom) fixes pull-quote-style content floating at the top of a 1fr row, applied in quote-impact + stat-anchor archetypes) |
...
| `vti-slide-diagram-builder` | **0.7.0** | 0.6.0 (**`cycle` primitive** — `make_cycle(stages, hub, …)` renders a closed loop of 3–8 stages around a ring + centre hub + clockwise chevrons, native SVG token-driven, viewBox 1168×566 with no-clip margins; registered in primitives/meta/intents (`loop`/`feedback-loop`/`lifecycle`/`closed-loop`); codifies the previously hand-generated loop SVG) |
| `vti-slide-diagram-builder` (prev) | 0.6.0 | 0.5.0 (**M2 — depth pass**: `svg_open` injects shared `<defs>` — per-accent vertical gradients + soft `feDropShadow` + line-icon symbols; `svg_box`/`svg_box_filled` fill with gradient + carry the soft shadow; quadrant cells lifted with the same shadow; connectors darkened muted→ink-soft; `svg_icon()` helper available for M3. All colours via the token bridge — no hex literals) |
